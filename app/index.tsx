...
import { getAllTasks } from '../src/utils/handle-api';
...
export default function App() {
  ...
  const addTaskStore = useTaskStore((state) => state.addTaskStore);
  const updateTaskStore = useTaskStore((state) => state.updateTaskStore);
  const clearAllTasks = useTaskStore((state) => state.clearAllTasks);
  const editingTask = useTaskStore((state) => state.editingTask);
  const setEditingTask = useTaskStore((state) => state.setEditingTask);
  const modalVisible = useTaskStore((state) => state.isModalVisible);
  ...
  const handleSave = () => {
    ...
    if (editingTask) {
      updateTaskStore(editingTask._id, text, completed, formattedDate);
    } else {
      addTaskStore(text, completed, formattedDate);
    }
    resetForm();
  };
      ...
      <View style={styles.container}>
        ...
        <View style={styles.filterContainer}>
          {([
            ...
            <TouchableOpacity 
              key={item.id}
              style={[styles.filterButton, filter === item.id ? styles.filterButtonActive : styles.filterButtonInactive]} 
              onPress={() => setFilter(item.id)}
            >
              <Text style={filter === item.id ? styles.filterTextActive : styles.filterTextInactive}>{item.label}</Text>
            </TouchableOpacity>
          ))}
        </View>
